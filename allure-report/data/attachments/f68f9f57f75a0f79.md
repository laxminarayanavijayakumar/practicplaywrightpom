...
Call log:
  - waiting for locator('//a[text()="Samsung galaxy s7"]')
    - locator resolved to <a class="hrefch" href="prod.html?idp_=4">Samsung galaxy s7</a>
  ...
  - retrying click action
    ...
    at C:\Users\HARIKATHIK V\Desktop\sdet-playwright\tests\record.spec.js:14:59
```
...
```ts
  ...
  10 |     await page.locator('#loginusername').fill('pavanal')
  11 |     await page.locator('#loginpassword').fill('123@tesr')
  12 |     await page.locator('//button[@onclick="logIn()"]').click()
  13 |
> 14 |     await page.locator('//a[text()="Samsung galaxy s7"]').click()
     |                                                           ^ Error: locator.click: Test timeout of 30000ms exceeded.
  15 |     await page.waitForTimeout(5000);
  16 |     // expect(await page.locator('#logout2')).toBeVisible()
  17 |     
  18 | })
```