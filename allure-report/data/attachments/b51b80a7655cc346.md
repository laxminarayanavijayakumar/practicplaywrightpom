...
Call log:
  ...
  - retrying click action
    ...
    - scrolling into view if needed
    - done scrolling
    - <form>…</form> from <div tabindex="-1" role="dialog" id="logInModal" class="modal fade show" aria-labelledby="logInModalLabel">…</div> subtree intercepts pointer events
  - retrying click action
    - waiting 100ms
    ...
    at Homepage.addproducttocart (C:\Users\HARIKATHIK V\Desktop\sdet-playwright\pages\Homepage.js:14:32)
...
```ts
  ...
  13 |             if(productnames === await products.textContent()) {
> 14 |                 await products.click()
     |                                ^ Error: elementHandle.click: Element is not attached to the DOM
  15 |                 break;
  16 |             }
  17 |         }
  ...
  21 |             }
  22 |         })
  23 |         await this.page.locator(this.addtocartbton).click();
  24 |     }
  25 |     
  26 |     async gotocart() {
  27 |         await this.page.locator(this.cart).click();        
  28 |     }
  29 | }
```