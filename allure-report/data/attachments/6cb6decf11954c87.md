...
Expected: 4
Received: {Symbol(async_id_symbol): 3375, Symbol(trigger_async_id_symbol): 3296, Symbol(kResourceStore): undefined}
    at C:\Users\HARIKATHIK V\Desktop\sdet-playwright\tests\webtable.spec.js:17:35
```
...
- table:
  - rowgroup:
    - row "Name CPU (%) Network (Mbps) Memory (MB) Disk (MB/s)":
      ...
      - cell "Disk (MB/s)"
  - rowgroup:
    - row "System 8.2% 8.0 Mbps 70.3 MB 0.82 MB/s":
      - cell "System"
      - cell "8.2%"
      - cell "8.0 Mbps"
      - cell "70.3 MB"
      - cell "0.82 MB/s"
    - row "Internet Explorer 5.4% 8.5 Mbps 97.6 MB 0.29 MB/s":
      - cell "Internet Explorer"
      - cell "5.4%"
      - cell "8.5 Mbps"
      - cell "97.6 MB"
      - cell "0.29 MB/s"
    - row "Firefox 9.7% 5.1 Mbps 71.5 MB 0.72 MB/s":
      - cell "Firefox"
      - cell "9.7%"
      - cell "5.1 Mbps"
      - cell "71.5 MB"
      - cell "0.72 MB/s"
    - row "Chrome 9.6% 9.7 Mbps 67.5 MB 0.65 MB/s":
      - cell "Chrome"
      - cell "9.6%"
      - cell "9.7 Mbps"
      - cell "67.5 MB"
...
```ts
  ...
  13 |
  14 |     const rows = await paginatable.locator('tbody tr')
  15 |     console.log('number of rows:',await rows.count())
  16 |
> 17 |     await expect(columns.count()).toBe(4)
     |                                   ^ Error: expect(received).toBe(expected) // Object.is equality
  18 |     await expect(rows.count()).toBe(5)
  19 |     
  20 | })
```