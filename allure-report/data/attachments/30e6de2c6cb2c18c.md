...
Call log:
  - waiting for locator('#singleFileInput')
    ...
    at C:\Users\HARIKATHIK V\Desktop\sdet-playwright\tests\uploadfiles.spec.js:7:44
```
...
- table:
  - rowgroup:
    - row "Name Memory (MB) CPU (%) Disk (MB/s) Network (Mbps)":
      ...
      - cell "Network (Mbps)"
  - rowgroup:
    - row "Chrome 36.5 MB 4.1% 0.64 MB/s 0.5 Mbps":
      - cell "Chrome"
      - cell "36.5 MB"
      - cell "4.1%"
      - cell "0.64 MB/s"
      - cell "0.5 Mbps"
    - row "System 36.4 MB 5.6% 0.57 MB/s 1.6 Mbps":
      - cell "System"
      - cell "36.4 MB"
      - cell "5.6%"
      - cell "0.57 MB/s"
      - cell "1.6 Mbps"
    - row "Firefox 80.6 MB 5.3% 0.36 MB/s 6.1 Mbps":
      - cell "Firefox"
      - cell "80.6 MB"
      - cell "5.3%"
      - cell "0.36 MB/s"
      - cell "6.1 Mbps"
    - row "Internet Explorer 66.2 MB 4.5% 0.49 MB/s 2.0 Mbps":
...
```ts
   ...
   6 |
>  7 |     await page.locator('#singleFileInput').check()
     |                                            ^ Error: locator.check: Error: Not a checkbox or radio button
   8 |
   9 |     await page.locator('#singleFileInput').setInputFiles('tests/uploadfiles/testfile1.pdf')
  10 |
  11 |     // await page.locator('(//button[@type="submit"])[1]').click()
  12 |
  13 |     await page.waitForTimeout(5000);
  14 |     
  15 | })
  16 |
  17 | test('multiple files upload', async ({page}) => {
  18 |
  19 |     await page.goto('')
  20 |     
  21 | })
