...
Call log:
  - waiting for locator(':scope')
    ...
    at C:\Users\HARIKATHIK V\Desktop\sdet-playwright\tests\screenshot2.spec.js:18:53
```
...
```ts
  ...
  17 |
> 18 |             if ('Sony xperia z5' === await products.textContent()) {
     |                                                     ^ Error: elementHandle.textContent: Execution context was destroyed, most likely because of a navigation
  19 |                 
  20 |                 await products.click()
  21 |                 break;
  22 |             }
  23 |         }
  24 |
  25 |         await page.on('dialog', async dialog=>{
  26 |             expect(dialog.type()).toContain('alert')
  27 |             expect(dialog.message()).toContain('I am an alert box!')
  28 |             if (dialog.message().inclides('Product added.')) {
  29 |                 await dialog.accept();
  30 |             }
  31 |         })
  32 |
  33 |         await page.locator('//a[@class="btn btn-success btn-lg"]').click();
  34 | })
```